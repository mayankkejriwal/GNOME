...
class ContingentAttributeNovelty(AttributeNovelty):
    def __init__(self):
        super(ContingentAttributeNovelty, self).__init__()
...
class GranularityRepresentationNovelty(RepresentationNovelty):
    def __init__(self):
        super(GranularityRepresentationNovelty, self).__init__()
    ...
    def granularity_novelty(self, current_gameboard, location, new_end_position):
        ...
        for loc in current_gameboard['location_sequence']:
            new_start_position = len(
                new_location_sequence)  # the new start position is the current length of the new sequence
            new_end_position = new_start_position + (
            loc.end_position - loc.start_position)  # the new end position is the new start position + difference
            for i in range(loc.start_position, loc.end_position):
                new_location_sequence.append(loc)
                if loc.loc_class == 'railroad':
                    current_gameboard['railroad_positions'].append(len(
                        new_location_sequence) - 1)  # we cannot directly use the start and end position, since the whole board is changing through 'stretching'
